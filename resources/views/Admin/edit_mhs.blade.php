            ...
            <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                ...
                <form action="{{ route('admin.mhs.update', $mahasiswa['npm']) }}" method="POST">
                    ...
                    <select name="id_user" class="w-full p-2 border rounded mb-4 @error('id_user') border-red-500 @enderror" required>
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u['id_user'] }}" {{ old('id_user', $mahasiswa['username']) == $u['username'] ? 'selected' : '' }}>
                                {{ $u['username'] }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="kode_kelas" class="w-full p-2 border rounded mb-4 @error('kode_kelas') border-red-500 @enderror" required>
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k['kode_kelas'] }}" {{ old('kode_kelas', $mahasiswa['nama_kelas']) == $k['nama_kelas'] ? 'selected' : '' }}>
                                {{ $k['kode_kelas'] }}
                            </option>
                        @endforeach
                    </select>
