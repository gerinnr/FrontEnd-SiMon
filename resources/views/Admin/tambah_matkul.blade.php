            ...
            <div class="flex justify-center items-center flex-grow">
                <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                    ...
                    <form action="{{ route('admin.matkul.store') }}" method="POST">
                        ...
                        <input type="text" name="kode_matkul" class="w-full p-2 border rounded mb-4 @error('kode_matkul') border-red-500 @enderror" required>
                        @error('kode_matkul')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror

                        <label class="block mb-2">Nama Mata Kuliah</label>
                        <input type="text" name="nama_matkul" class="w-full p-2 border rounded mb-4 @error('nama_matkul') border-red-500 @enderror" required>
                        @error('nama_matkul')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror

                        <label class="block mb-2">SKS</label>
                        <input type="number" name="sks" class="w-full p-2 border rounded mb-4 @error('sks') border-red-500 @enderror" required>
                        @error('sks')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
