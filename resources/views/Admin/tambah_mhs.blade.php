    ...
    <div class="flex-1 p-5 flex flex-col">
        ...
        <div class="flex justify-center items-center flex-grow">
            <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                <h2 class="text-xl font-bold text-center mb-4">Tambah Data Mahasiswa</h2>
                ...
                <form action="{{ route('admin.mhs.store') }}" method="POST">
                    ...
                    <input type="number" name="npm" class="w-full p-2 border rounded mb-4 @error('npm') border-red-500 @enderror" required>
                    @error('npm') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                    <label class="block mb-2">Nama Mahasiswa</label>
                    <input type="text" name="nama_mahasiswa" class="w-full p-2 border rounded mb-4 @error('nama_mahasiswa') border-red-500 @enderror" required>
                    @error('nama_mahasiswa') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                    <label class="block mb-2">Email</label>
                    <input type="email" name="email" class="w-full p-2 border rounded mb-4 @error('email') border-red-500 @enderror" required>
                    ...
                    @error('id_user') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                    <label class="block mb-2">Pilih Kode Kelas</label>
                    <select name="kode_kelas" class="w-full p-2 border rounded mb-4 @error('kode_kelas') border-red-500 @enderror" required>
                    ...
                    @error('kode_kelas') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded w-full">Tambah Data</button>
                </form>

                <a href="{{ route('admin.mhs.index') }}" class="block text-center mt-4 text-blue-500">Kembali</a>
            </div>
        </div>
    </div>
</div>
