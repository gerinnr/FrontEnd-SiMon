...
<body class="flex items-center justify-center h-screen bg-gray-100">
    <div class="bg-[#e8e8d8] p-10 rounded-lg shadow-lg w-full max-w-3xl h-[70vh] flex flex-col items-center gap-10">
        <h2 class="text-3xl font-bold mb-6">LOGIN ADMIN</h2>
        <div class="flex flex-col md:flex-row items-center gap-10 w-full justify-center">
            ...
            <form action="{{ route('login.auth') }}" method="POST" class="flex flex-col gap-4 w-64">
                @csrf
                <input type="text" name="username" placeholder="Username" class="p-3 border rounded-lg shadow-sm w-full" required>
                <input type="password" name="password" placeholder="Password" class="p-3 border rounded-lg shadow-sm w-full" required>
                ...
                <button type="submit" class="bg-red-700 text-white p-3 rounded-lg shadow-md hover:bg-red-800 text-center w-full">
                    LOG IN
                </button>
            </form>
        </div>
